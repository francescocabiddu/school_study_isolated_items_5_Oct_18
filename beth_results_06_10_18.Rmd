...
isolated_plot <- function(df, title) {
  df %>%
    group_by(id, age_month, stimulus_type) %>%
    mutate(recalled = as.integer(recalled)) %>%
    ...
    labs(x = "Age in months", y = "Number of items accurately recalled") +
...
isolated_plot(df_isolated_item, "Isolated items")
```
...
#### isolated pairs plot
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=7, fig.height=5}
isolated_plot(df_isolated_pair, "Isolated pairs")
```
